// Undertone visuals — one INSTANCE per Visual (call create()), so several visuals on screen
// (panel + one screensaver per monitor) never share or double-step the same state.
.pragma library
function create() {
  ...
  return { setup: setup, resetVis: resetVis, setPalette: setPalette, setState: setState, setBreath: setBreath, hit: hit,
           frame: frame, paintRects: paintRects, probe: probe, useOwnBuffer: useOwnBuffer, toBmp: toBmp, toBmpBase64: toBmpBase64,
           bytesToB64: bytesToB64, bufferSize: bufferSize, stepLavaOnly: stepLavaOnly, tickFlash: tickFlash, setAudio: setAudio };
}
